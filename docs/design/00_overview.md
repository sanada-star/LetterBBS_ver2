...
ver1（現行版）は KENT WEB「Patio」ベースに段階的改造を重ねた結果、以下の問題を抱えている：

- **データ破損リスク**: フラットファイル＋flock による排他制御の限界
- **コードの一貫性欠如**: ツギハギ的改造による関数の重複、命名規則の不統一
- **JS の世代混在**: bbs.js / bbs_v3.js / bbs_v4.js が共存

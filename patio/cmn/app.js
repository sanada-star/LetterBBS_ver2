...
(function (window, document) {
  ...
  var LB = (window.LB = window.LetterBBS = {});
  ...
  LB.API = {
    ...
    deleteDraft: function (draftId) {
      return this.request("desk_delete", { draft_id: draftId, csrf_token: LB.Config.csrfToken }, "POST");
    },
    sendDrafts: function (draftIds, password) {
      return this.request(
        "desk_send",
        { draft_ids: draftIds, password: password, csrf_token: LB.Config.csrfToken },
        "POST",
      );
    },
  };
  ...
  LB.Desk = {
    ...
    updateBadge: function() {
      var badge = document.getElementById('desk-badge');
      if (!badge) return;
      var count = this.drafts.length;
      if (count > 0) {
    ...
    addFromThread: function (threadId, author, btn) {
      ...
      if (area.style.display === "none" || area.style.display === "") {
        ...
        if (timelineArea) {
          // タイムラインの検索キーとしては、現在のスレッド主（自分）とレスの投稿者（相手）の組み合わせにする
          var queryMyName =
            LB.Config.threadAuthor || (nameInput ? nameInput.value : "");
          if (queryMyName && author) {
            LB.API.getTimeline(queryMyName, author)
              .then(function (data) {
    ...
    // パネル表示/非表示切り替え
    togglePanel: function(forceShow) {
      if (typeof forceShow !== 'undefined') {
        this.panelVisible = forceShow;
      } else {
      ...
      if (this.panelVisible) {
        document.body.classList.add('desk-split-mode');
        this.refreshPanel();
      } else {
        document.body.classList.remove('desk-split-mode');
      }
    },
        ...
        .then(function (data) {
          ...
          LB.Desk.drafts.forEach(function (draft) {
            ...
            item.innerHTML =
              ...
              "<strong>" +
              LB.UI.escapeHtml(draft.thread_subject || "(無題)") +
              "</strong>" +
              '<span class="desk-draft-meta">' +
              LB.UI.escapeHtml(draft.author) +
              " → " +
              LB.UI.escapeHtml(draft.thread_author) +
              "</span>" +
              "</div>" +
              '<div class="desk-draft-body">' +
              LB.UI.escapeHtml(draft.body).substring(0, 80) +
              "</div>" +
              '<div class="desk-draft-actions">' +
              '<button onclick="LB.Desk.removeDraft(' +
              draft.id +
              ')" class="btn-cancel-desk">削除</button>' +
              "</div>";
    ...
    sendAll: function () {
      ...
      if (!LB.UI.confirm(this.drafts.length + "件の返信を一括送信しますか？"))
        return;
  ...
  LB.Timeline = {
    ...
    renderInto: function (container, posts) {
      ...
      posts.forEach(function (post) {
        var div = document.createElement("div");
        div.className =
          "timeline-msg timeline-" + (post.direction || "received");
        div.innerHTML =
          '<div class="timeline-meta">' +
          '<span class="timeline-author">' +
          LB.UI.escapeHtml(post.author) +
          "</span>" +
          ...
          LB.UI.escapeHtml(post.body || "").substring(0, 200) +
          "</div>";
  ...
  LB.Notify = {
    ...
    init: function () {
      try {
        ...
        this.tasks = JSON.parse(
          localStorage.getItem("lb_notify_tasks") || "[]",
        );
      } catch (e) {
        this.watchName = "";
        this.tasks = [];
    ...
    poll: function () {
      var self = this;
      LB.API.getThreads(this.lastCheck)
        .then(function (data) {
          self.lastCheck = data.server_time;

          (data.threads || []).forEach(function (thread) {
            if (
              thread.last_author !== self.watchName &&
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (window.LB_CONFIG) {
      ...
      LB.Config.threadAuthor = window.LB_CONFIG.threadAuthor || "";
    }
